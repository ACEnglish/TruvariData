...
def match_calls(base_entry, comp_entry, astart, aend, sizeA, sizeB, reference, args, outputs):
    ...
    Returns True if it failed to match, False if they shouldn't(?)
    Returns a match otherwise
    """
    comp_start, comp_end = truvari.entry_boundaries(comp_entry)
    comp_type = truvari.entry_variant_type(comp_entry)

    base_start, base_end = truvari.entry_boundaries(base_entry)
    base_type = truvari.entry_variant_type(base_entry)
    ...
    ovl_pct = truvari.reciprocal_overlap(comp_start, comp_end, base_start, base_end)
    if ovl_pct < args.pctovl:
        return False

    start_distance = base_start - comp_start
    end_distance = base_end - comp_end

    score = truvari.weighted_score(1, 1, ovl_pct)

    return MATCHRESULT(score, 1, 1, ovl_pct, 0,
